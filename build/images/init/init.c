...
static void write_resolv_conf(void) {
    ...
    FILE *f = fopen(path, "w");
    if (!f) {
        print_error("open resolv.conf");
        return;
    }
    const char *contents =
        "# Generated by volant init\n"
        "nameserver 8.8.8.8\n"
        "nameserver 1.1.1.1\n";
    if (fwrite(contents, 1, strlen(contents), f) != strlen(contents)) {
        print_error("write resolv.conf");
    }
    fclose(f);
}
...
int main(void) {
    ...
    // Ensure DNS resolution is configured before network services start
    write_resolv_conf();
