...
static void configure_network(const char *cmdline) {
  ...
  if (!p) {
    fprintf(stderr, "Viper Init Info: missing ip= param, skipping static "
                    "network config.\n");
  ...
  if (addr && netmask) {
    snprintf(cmd, sizeof(cmd), "ip addr add %s/%s dev %s", addr, netmask,
             iface);
    run_cmd(cmd);
  }
  if (gateway) {
    snprintf(cmd, sizeof(cmd), "ip route add default via %s dev %s", gateway,
             iface);
    run_cmd(cmd);
  }
